...
1. Let's check the pod in details
    ...
    > It seems the pod is looking for a config-map that's not available and mount is failed
...
3. If you notice carefully, there is mismatch between name. The actual config-map name is: `redis-config` but pod is lokking for `redis-cofig` and image name is `redis-alpin` which should be `redis-alpine`
...
5. Let's fix the deployment file `cofig` to `config` in volumes and image: `redis-alpin` to `redis-alpine`
   
6. Delete existing deployment
   ```
   kubectl delete deployment
   ```
7. Run the new deployment file
