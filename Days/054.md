...
- The file `beta.txt` should be present under the mounted path `/tmp/demo` on the second container `volume-container-nautilus-2` as well, since they are using a shared volume.
...
## Logical Structure
        ...
        │ /tmp/beta              │     │     │ /tmp/demo              │
        │                        │     │     │                        │
        │ sees files here        │─────┼────▶│ sees SAME files here  │
        │                        │     │     │                        │
        └────────────────────────┘     │     └────────────────────────┘
                    │                  │
                    └──────────────────┘
...
1. Let's create the `k3s-pod.yml` file using the below content
...
4. Execute the first container

    ```sh
    kubectl exec -it volume-share-nautilus -c volume-container-nautilus-1 -bash
    ```

    > Create `beta.txt` file with content for verfication

    ```sh
     echo "Hello shared storage" /tmp/beta/beta.txt
    ```

5. Execute the 2nd container
