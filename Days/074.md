...
4. Copy the `db_$(date +%F).sql` dump to the Backup Server under location `/home/natasha/db_backups`.
...
2. Go to Manage Jenkins > System > Add SSH servers > Add details of databse server
   - **Name**: `stdb01`  
...
3. Create a freestyle job named: `database-backup`:
    ...
    sshpass -p 'H@wk3y3' scp -o StrictHostKeyChecking=no /tmp/db-backup/*.sql clint@stbkp01:/home/natasha/db_backups
