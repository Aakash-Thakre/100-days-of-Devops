...
4. Create a Jenkins pipeline job named `datacenter-webapp-job` (it must not be a `Multibranch pipeline`) and configure it to:
    ...
    - It should conditionally deploy the code from `web_app` repository under `/var/www/html` on App Server 1, as this location is already mounted to the document root `/var/www/html` of app servers. The pipeline should have a single stage named `Deploy` ( which is case sensitive ) to accomplish the deployment.
...
1. Follow the steps done in Day 077.md file just need to update the job
2. During creating the job, select `This is parameterized` and add string parameter:
   
    - Name: `BRANCH`
    - Default Value: `master`

3. Update jenkins pipeline with this below script
   ```groovy
    pipeline {
        agent { label 'stapp01' }
            ...
            stage('Deploy') {
                steps {
                    script {
                        sh '''
                            rm -rf /tmp/web_app
                            git clone -b ${BRANCH}  "updated git url paste here" /tmp/web_app
