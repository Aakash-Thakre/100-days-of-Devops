...
2. Create a jenkins user named `anita` with the password `dCV3szSGNA`. Their full name should match `Anita`.
...
1. Manage Jenkins > Users > Create user > set username, full name, password to create user
2. Manage Jenkins > Plugins > Available Plugins > Search Matrix Authorization Strategy > Install it > restart jenkins
3. Manage Jenkins > Security > Authorization > Select Project-based Matrix authorization Strategy > Update permission like for Admin | overall `administer` and Anita overall `read` > save and apply
4. Dashboard > HelloWorld > Configuration > Enable Project-based security > add user `Anita` > give only read permission > Save and Apply
